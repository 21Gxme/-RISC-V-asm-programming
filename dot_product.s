...
# }
.data
a:  .word 1, 2, 3, 4, 5   # Array 'a' containing 5 elements
b:  .word 6, 7, 8, 9, 10  # Array 'b' containing 5 elements
dot: .string "The dot product is: "
newline: .string "\n"

.text
main:
    la t0, a    # Load the 'a' into t0
    la t1, b    # Load the 'b' into t1
    li s0, 5    # len of array
    addi s1 , x0, 0 # set s1 = i
    addi s2 , x0, 0 # sop = 0

loop:
    bge s1, s0, exit # if i >= 5 go to exit
    slli x18, s1, 2 # x18 t0 i*4
    add x19, x18, t0 # x19 = i*4 + a
    lw x20, 0(x19) # a[i]
    add x21, x18, t1 # x19 = i*4 + b
    lw x22, 0(x21) # b[i]
    mul x23, x20, x22 # a[i] * b[i]
    add s2, s2, x23 # sop += a[i] * b[i]
    addi s1, s1, 1 # i++
    j loop
